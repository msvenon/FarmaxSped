...
uses
    Classes, ACBrSpedPisCofins, SqlExpr, SysUtils, StdCtrls, DB, DBClient, Provider, ComCtrls, System.StrUtils,
    ...
    ACBrNFe, ACBrNFeNotasFiscais, pcnNFe, pcnConversao,  pcnConversaoNFe, Math, ACBrECF, ACBrDevice,
    IniFiles, TypInfo, ACBrSpedFiscal, System.Threading,System.Generics.Collections;
...
type
    ...
    private
      ...
      // teste para carregar o documento xml mas rapido abaixa  mauricio 23092042




      // fim do crregamento de xml
...
uses udmPrincipal, uMenuPrincipal, Vcl.Controls,
  ...
  uSpedFiscal, Vcl.Dialogs, UFrmConsistencia,Winapi.Windows;
...
procedure TSpedPisCofins.AcumularValoreAnaliticoRegC175(const NFID: Integer; const CFOP, CST_PIS_COF: String; const ValorBase,
   ...
   begin
      FTabelaRegC175.IndexFieldNames:='NFID';
      FTabelaRegC175.IndexFieldNames:='CFOP';
      FTabelaRegC175.IndexFieldNames:='CST_PIS';

      FTabelaRegC175.Filtered := False;
      FTabelaRegC175.Filter := 'NFID = ' + IntToStr(NFID) + ' and ' +
                               'CFOP = ' + QuotedStr(Trim(CFOP)) + ' and ' +
                               'CST_PIS = ' + QuotedStr(CST_PIS_COF);
      FTabelaRegC175.Filtered := True;
      Result := (FTabelaRegC175.RecordCount > 0);
   end;
...
begin
   ...
   if (ExisteCombinacaoCST_PIS_COF()) then
      ...
      FTabelaRegC175.Append;

   if (FTabelaRegC175.State = dsInsert) then
      begin
         FTabelaRegC175.FieldByName('NFID').AsInteger := NFID;
         FTabelaRegC175.FieldByName('CFOP').AsString := Trim(CFOP);
         FTabelaRegC175.FieldByName('CST_PIS').AsString := CST_PIS_COF;
         FTabelaRegC175.FieldByName('CST_COFINS').AsString := CST_PIS_COF;
         FTabelaRegC175.FieldByName('ALIQ_PIS').AsFloat := AliqPis;
         FTabelaRegC175.FieldByName('ALIQ_COFINS').AsFloat := AliqCof;

         FTabelaRegC175.FieldByName('COD_CTA').AsString := '';
         FTabelaRegC175.FieldByName('INFO_COMPL').AsString := '';
      end;

   // Valor Receita
   FTabelaRegC175.FieldByName('VL_OPR').AsFloat := FTabelaRegC175.FieldByName('VL_OPR').AsFloat + ValorBase;
   FTabelaRegC175.FieldByName('VL_DESC').AsFloat := FTabelaRegC175.FieldByName('VL_DESC').AsFloat + ValorDesc;

   if (AliqPis > 0) then
      begin
         FTabelaRegC175.FieldByName('VL_PIS').AsFloat := FTabelaRegC175.FieldByName('VL_PIS').AsFloat + ValorPis;
         FTabelaRegC175.FieldByName('VL_BC_PIS').AsFloat := FTabelaRegC175.FieldByName('VL_BC_PIS').AsFloat + ValorBase;
      end;

   if (AliqCof > 0) then
      begin
         FTabelaRegC175.FieldByName('VL_COFINS').AsFloat := FTabelaRegC175.FieldByName('VL_COFINS').AsFloat + ValorCof;
         FTabelaRegC175.FieldByName('VL_BC_COFINS').AsFloat := + FTabelaRegC175.FieldByName('VL_BC_COFINS').AsFloat + ValorBase;
      end;
...
procedure TSpedPisCofins.AcumularValoresReceitaReg1900(const CFOP, CST_PIS_COF, ModDoc, Ser, SubSer: String; const ValorReceita, AliqPis, AliqCof: Double);
   ...
   function ExisteCombinacaoCST_PIS_COF: Boolean;
   begin
      FTabelaReg1900.Filtered := False;
      FTabelaReg1900.Filter := 'CFOP = ' + QuotedStr(Trim(CFOP)) + ' and ' +
                               'CST_PIS_COF = ' + QuotedStr(CST_PIS_COF) + ' and ' +
                               'COD_MOD = ' + QuotedStr(ModDoc);
      FTabelaReg1900.Filtered := True;
      Result := (FTabelaReg1900.RecordCount > 0);
   end;
...
begin
   ...
   if (ExisteCombinacaoCST_PIS_COF()) then
      ...
      FTabelaReg1900.Append;

   if (FTabelaReg1900.State = dsInsert) then
      begin
         FTabelaReg1900.FieldByName('CFOP').AsString := Trim(CFOP);
         FTabelaReg1900.FieldByName('CST_PIS_COF').AsString := CST_PIS_COF;
         FTabelaReg1900.FieldByName('COD_MOD').AsString := ModDoc;
      end;

   // Valor Receita
   FTabelaReg1900.FieldByName('VL_TOT_REC').AsFloat := FTabelaReg1900.FieldByName('VL_TOT_REC').AsFloat + ValorReceita;
   FTabelaReg1900.FieldByName('SER').AsString := Ser;
   FTabelaReg1900.FieldByName('SUB_SER').AsString := SubSer;
   FTabelaReg1900.FieldByName('ALIQ_PIS').AsFloat := AliqPis;
   FTabelaReg1900.FieldByName('ALIQ_COF').AsFloat := AliqCof;
...
begin
   ...
   EnquandramentoTributario := dmPrincipal.cdsConsEmpresaENQUADRAMENTO_TRIBUTARIO.value ;
   ...
   for Idx := 0 to Notas.Items[ 0 ].NFe.Det.Count -1 do
      begin
        ...
        if (Notas.Items[0].NFe.Ide.modelo = 65) then  //02082024
           begin
         ...
         if (Notas.Items[0].NFe.Ide.modelo <> 65) then
            begin
                  ...
                  UniMedida:=UpperCase(COPY(oItemProduto.Prod.uCom,1,2));

               if (UniMedida <>  'UN')  then
                  begin
                   UniMedida :=  'UN';
                   end;
         ...
         if (not docEntrada) then
            begin
               ...
               // A impressora fiscal não fornece os calculos dos Impostos de Pis e Cofins
               //if Self.FGerarCupomFiscal then
               //   AcumularValoresAnaliticosRecC481C485();
            end;
      end;// for
end;
   ...
   try
      ...
      if (FTabelaRegC100.IsEmpty) then
         begin
            ...
            Result := FTabelaRegC100.RecordCount + 1; // Atualiza o resultado
            FTabelaRegC100.FieldByName('ID').AsInteger := Result;


            if docEntrada or (Notas.Items[0].NFe.Ide.tpNF = tnEntrada) then
              FTabelaRegC100.FieldByName('IND_OPER').AsInteger := 0 // Entrada
            else
              FTabelaRegC100.FieldByName('IND_OPER').AsInteger := 1; // Saída
...
begin
   ...
   for Idx := 0 to Notas.Items[ 0 ].NFe.Det.Count -1 do
      begin
         ...
         try
            unidade:= UpperCase(COPY(oItemProduto.Prod.uCom,1,2));
...
begin
   ...
   for Idx := 0 to Notas.Items[ 0 ].NFe.Det.Count -1 do
      begin
        ...
        with dmPrincipal.cdsTemCstPIsCof do
           begin
             ...
             case oItemProduto.Imposto.PIS.CST of
                 ...
                 pis01:
                  ...
                  CST_PIS := '49'; // valor padrão

             end;

               CST_PIS := CST_PIS; // Atribui o valor de CST_PIS
               post; // Aplica atualizações após todas as insercao
           end;
                     ...
                     begin
                      IF (unidade =  'CX') OR
                       ...
                       begin
                        ...
                        FTabelaReg0190.Append;
                        FTabelaReg0190.FieldByName('UNID').AsString := unidade;
                        FTabelaReg0190.FieldByName('DESCR').AsString := 'UNIDADE MEDIDA ' + 'CX';
                        FTabelaReg0190.Post;
                       end;
                     end;
               finally
                ...
                //  GerarLinhaMemoLog('Lendo informações do documento: ' + sChave);
                  ...
                  if InformacoesNFeValidasParaEscriturar(FacbrNFe.NotasFiscais) then
                     begin
                        ...
                        iIDNota := AdicionarDadosNotas(FacbrNFe.NotasFiscais);    //aqui
                        if (iIDNota > 0) then;
                           begin
                              // Adicionar itens
...
begin
   ...
   while not FTabelaReg0190.Eof do
      begin
         ...
         if(unidade =  'CX') OR
           (unidade =  'UN') OR
           (unidade =  'PC') OR
           (unidade =  'BI') OR     //mauricio 05062024   mudORça pra tratar erro de unidade de medida
           (unidade =  'BL') OR
           (unidade =  'CT') OR
           (unidade =  'DP') OR
           (unidade =  'DZ') OR
  ...
  // GerarLinhaMemoLog(_SPED_PIS_COF_BLOCO_0 + ': Gerando Registro 0400 - Natureza de operações - CFOP');
   ...
   AtualizarStatus('Gerando Planos de Contas Contábeis ');
   if (
...
begin
   ...
   if (FGerarBlocoF) then
      begin
         FTabelaReg1900.First;
         while not FTabelaReg1900.Eof do
            begin
               Registro1900 := FcompSpedPisCofins.Bloco_1.Registro1900New;

               Registro1900.CNPJ := Self.CnpjEmpresa;

               Registro1900.COD_MOD := FTabelaReg1900.FieldByName('COD_MOD').AsString;
               Registro1900.SER := FTabelaReg1900.FieldByName('SER').AsString;;
               Registro1900.SUB_SER := FTabelaReg1900.FieldByName('SUB_SER').AsString;;
              ...
              Registro1900.COD_SIT := csffRegular;
               ...
               Registro1900.VL_TOT_REC := FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency; {Valor total por modelo de documento}
               Registro1900.QUANT_DOC := FTabelaReg1900.RecordCount; {Quantidade por modelo de documentos}
               ...
               Registro1900.CST_PIS := ACBrEPCBlocos.StrToCstPis(FTabelaReg1900.FieldByName('CST_PIS_COF').AsString);

               {
                  Existindo mais de um CST para o modelo de documento consolidado no registro, sem que a pessoa jurídica tenha
                 como decompor por cada CST, o campo CST_COFINS será escriturado em branco.
               }
               Registro1900.CST_COFINS := ACBrEPCBlocos.StrToCstCofins(FTabelaReg1900.FieldByName('CST_PIS_COF').AsString);
               Registro1900.CFOP := FTabelaReg1900.FieldByName('CFOP').AsInteger;
...
begin
   GerarLinhaMemoLog(_SPED_PIS_COF_BLOCO_C + ': Gerando Registros C001');
    ...
    AtualizarStatus('Gerando Registros C001');
   ...
   AtualizarStatus('Gerando Registros C010');
   ...
   AtualizarStatus('Gerando Registros C100');


 // FACBrSPEDECF.WriteBloco_C(false);
   ...
   while not FTabelaRegC100.Eof do
      begin
         ...
         if (RegistroC100.COD_SIT = ACBrEPCBlocos.sdRegular) then
            begin
               {Na escrituração das NFC-e no registro C100, não precisam ser informados os campos:
                   COD_PART, VL_BC_ICMS_ST, VL_ICMS_ST, VL_IPI, VL_PIS, VL_COFINS, VL_PIS_ST e VL_COFINS_ST.F }
               if (RegistroC100.COD_MOD = '65') then
                ...
                {Registro C170}
                FTabelaRegC170.IndexFieldNames:='NFID';
               ...
               cTotalPIS_Itens := 0;
               cTotalCOF_Itens := 0;
               AtualizarStatus('Gerando Registros C170');
               iSequencialItem := 1;
               ...
               // Registro - C175 - visão analítica, por CST
                   AtualizarStatus('Gerando Registros C175 - Analítico p/ CST NFC-e');
               if (RegistroC100.COD_MOD = '65') then
                  begin
                     ...
                     FTabelaRegC175.IndexFieldNames:='NFID';
                     FTabelaRegC175.Filtered := False;
                     FTabelaRegC175.Filter   := 'NFID = ' + FTabelaRegC100.FieldByName('ID').AsString;
                     FTabelaRegC175.Filtered := True;

                     FTabelaRegC175.First;
                     while not FTabelaRegC175.Eof do
                        begin
                           RegistroC175 := FcompSpedPisCofins.Bloco_C.RegistroC175New;

                           RegistroC175.CFOP := FTabelaRegC175.FieldByName('CFOP').AsString;
                           RegistroC175.VL_DESC := FTabelaRegC175.FieldByName('VL_DESC').AsCurrency;

                           RegistroC175.VL_OPR := RoundTo(FTabelaRegC175.FieldByName('VL_OPR').AsCurrency, -2);
                           cTotalOperacoes := cTotalOperacoes + RegistroC175.VL_OPR;

                           // *** PIS
                           RegistroC175.CST_PIS := ACBrEPCBlocos.StrToCstPis(FTabelaRegC175.FieldByName('CST_PIS').AsString);
                           RegistroC175.ALIQ_PIS := FTabelaRegC175.FieldByName('ALIQ_PIS').AsCurrency;
                           RegistroC175.VL_PIS := RoundTo(FTabelaRegC175.FieldByName('VL_PIS').AsCurrency, -2);

                           if (RegistroC175.VL_OPR > 0) then //mauricio 24052024
                              RegistroC175.VL_BC_PIS := FTabelaRegC175.FieldByName('VL_BC_PIS').AsCurrency
                           else
                              RegistroC175.VL_BC_PIS := 0;

                           // *** COFINS
                           RegistroC175.CST_COFINS  := ACBrEPCBlocos.StrToCstCofins(FTabelaRegC175.FieldByName('CST_COFINS').AsString);
                           RegistroC175.ALIQ_COFINS := FTabelaRegC175.FieldByName('ALIQ_COFINS').AsCurrency;
                           RegistroC175.VL_COFINS   := RoundTo( ((FTabelaRegC175.FieldByName('ALIQ_COFINS').AsCurrency * FTabelaRegC175.FieldByName('VL_BC_COFINS').AsCurrency)/100),-2);//mauricio 29/05/2024

                           /// RoundTo(FTabelaRegC175.FieldByName('VL_COFINS').AsCurrency, -2);

                           if (RegistroC175.VL_OPR > 0) then
                              RegistroC175.VL_BC_COFINS := FTabelaRegC175.FieldByName('VL_BC_COFINS').AsCurrency
                           else
...
begin
   ...
   AtualizarStatus('Registros Bloco D');
            ...
            begin
               ...
               while not FTabelaReg1900.Eof do
                  begin
                     ...
                     RegistroF550.VL_REC_COMP := FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency;

                     {Pis}
                     RegistroF550.CST_PIS := ACBrEPCBlocos.StrToCstPis(FTabelaReg1900.FieldByName('CST_PIS_COF').AsString);
                     RegistroF550.VL_DESC_PIS := 0;

                     RegistroF550.VL_BC_PIS := 0;
                     RegistroF550.ALIQ_PIS := 0;
                     RegistroF550.VL_PIS := 0;
                     if (FTabelaReg1900.FieldByName('ALIQ_PIS').AsCurrency > 0) then
                        begin
                           RegistroF550.VL_BC_PIS := FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency;
                           RegistroF550.ALIQ_PIS := FTabelaReg1900.FieldByName('ALIQ_PIS').AsCurrency;
                           RegistroF550.VL_PIS := (FTabelaReg1900.FieldByName('ALIQ_PIS').AsCurrency *
                                                   FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency);
                        end;

                     {Cofins}
                     RegistroF550.CST_COFINS := ACBrEPCBlocos.StrToCstCofins(FTabelaReg1900.FieldByName('CST_PIS_COF').AsString);
                     RegistroF550.VL_DESC_COFINS := 0;

                     RegistroF550.VL_BC_COFINS := 0;
                     RegistroF550.ALIQ_COFINS := 0;
                     RegistroF550.VL_COFINS := 0;
                     if (FTabelaReg1900.FieldByName('ALIQ_COF').AsCurrency > 0) then
                        begin
                           RegistroF550.VL_BC_COFINS := FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency;
                           RegistroF550.ALIQ_COFINS := FTabelaReg1900.FieldByName('ALIQ_COF').AsCurrency;
                           RegistroF550.VL_COFINS := (FTabelaReg1900.FieldByName('ALIQ_COF').AsCurrency *
                                                      FTabelaReg1900.FieldByName('VL_TOT_REC').AsCurrency);
                        end;

                     RegistroF550.COD_MOD := FTabelaReg1900.FieldByName('COD_MOD').AsString;
                     RegistroF550.CFOP := FTabelaReg1900.FieldByName('CFOP').AsInteger;
...
//                      CST_PIS          := ACBrEPCBlocos.stpisOperCredRecTribENaoTribMercIntEExportacao;
//                      VL_BC_PIS_TOT    := 0;
//                      VL_BC_PIS_CUM    := 0;
//                      VL_BC_PIS_NC     := 0;
//                      VL_BC_PIS        := 0;
             ...
             begin
                with RegistroM200New do
                   begin
                     ...
                     if FrmPrincipal.CbbIncidenciaTributaria.ItemIndex <> 1 then
                        begin
                           VL_TOT_CONT_NC_PER := StrtoFloat(FormatFloat('#0.00', (dmPrincipal.CdsSPEDM200VL_REC_BRT.AsFloat * AliqPis ) / 100));;
                        ...
                        begin
                         ...
                         while not dmPrincipal.CdsSPEDM210.Eof do
                            begin
                              VL_CONT_CUM_REC     := VL_TOT_CONT_CUM_PER + (dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat * 0.65) / 100;
                              VL_TOT_CONT_CUM_PER := VL_TOT_CONT_CUM_PER + (dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat * 0.65) / 100;
                              VL_TOT_CONT_REC     := VL_TOT_CONT_REC     + (dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat * 0.65) / 100;
                         ...
                         with RegistroM210New do
                             begin
                                ...
                                if FrmPrincipal.CbbIncidenciaTributaria.ItemIndex <> 1 then
                                 ...
                                 VL_BC_CONT        :=  dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat;   //dmPrincipal.CdsSPEDM210VL_BASE_CALCULO.AsFloat;
                                 VL_BC_CONT_AJUS   :=  dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat;   //dmPrincipal.CdsSPEDM210VL_BASE_CALCULO.AsFloat;
                                 VL_CONT_APUR      := (dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat * ALIQ_PIS) / 100;
                                 VL_AJUS_ACRES     := 0;
                                 VL_AJUS_REDUC     := 0;
                                 VL_CONT_DIFER     := 0;
                                 VL_CONT_DIFER_ANT := 0;
                                 VL_CONT_PER       := (dmPrincipal.CdsSPEDM210VL_REC_BRT.AsFloat * ALIQ_PIS) / 100;
                                 {QUANT_BC_PIS := 0;
                                 ALIQ_PIS_QUANT := 0;}
                             end;
                           dmPrincipal.CdsSPEDM210.Next;
...
//                         ALIQ_COFINS        := 3.0;
//
//                         VL_CRED        := 488.73;       //DmSPEDContribuicoes.CdsSPEDM500VL_COFINS.AsFloat; //OBRIGATORIO
//                         VL_AJUS_ACRES  := 0;
//                         VL_AJUS_REDUC  := 0;
//                         VL_CRED_DIFER  := 0;
//                         VL_CRED_DISP   := 488.73;       //DmSPEDContribuicoes.CdsSPEDM500VL_COFINS.AsFloat;
//                         IND_DESC_CRED  := idcTotal;
//                         VL_CRED_DESC   := 488.73;      //DmSPEDContribuicoes.CdsSPEDM500VL_COFINS.AsFloat; //Valor do Crédito disponível, descontado da contribuição apurada no próprio período. Se IND_DESC_CRED=0, informar o valor total do Campo 12; Se IND_DESC_CRED=1, informar o valor parcial do Campo 12.
//                         SLD_CRED       := 0;
//                       end;
//
//
...
//                        begin
//                            NAT_BC_CRED         := bccAqBensRevenda;
//                            CST_COFINS          := ACBrEPCBlocos.stcofinsCredPresAquiRecTribENaoTribMercIntEExportacao;
//                            VL_BC_COFINS_TOT    := 0;
//                            VL_BC_COFINS_CUM    := 0 ;
//                            VL_BC_COFINS_NC     := 0;
//                            VL_BC_COFINS        := 0;
...
var
 teste:string;
begin
 AtualizarStatus('Gerando RegistroM200 e RegistroM210...');
 dmPrincipal.FdQueryAuxiliar.sql.Clear;
...
//
 ...
 AtualizarStatus('Gerando RegistroM800 e RegistroM810...');
 dmPrincipal.FdQueryAuxiliar.sql.Clear;
